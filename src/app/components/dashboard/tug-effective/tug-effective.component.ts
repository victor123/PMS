...
@Component({
  selector: 'tug-effective',
  templateUrl: './tug-effective.component.html',
  styleUrls: [],
  providers: [PilotDemandDataService]
})
export class TugEffectiveComponent {
  ...
  public loadData(fromDate,fromtime,toDate,totime) {
    ...
    this.service.tugUserManualData(fromDate,fromtime,toDate,totime).subscribe((res) => {
      if (res.length > 0) {
        this.noData= false;
  ...
  public onFilter(value: any) {
    ...
    // if (valid) {
      this.service.tugUserManualFilerData(value, (res) => {
        console.log(res);
        if (res.length > 0) {
        this.noData= false;
        this.pieData = this.convertPercentage(res); 
  ...
  public onReset() {
    ...
    this.service.tugUserManualFilerData(this.filtersForm.value, (res) => {
        if (res.length > 0) {
          this.noData= false;
          this.pieData = this.convertPercentage(res); 
        } else {
